...
# from ItachiRobot.modules.sql import warns_sql as warnssql
import ItachiRobot.modules.sql.blacklist_sql as blacklistsql

# from ItachiRobot.modules.sql import cust_filters_sql as filtersql
# import ItachiRobot.modules.sql.welcome_sql as welcsql
import ItachiRobot.modules.sql.locks_sql as locksql
import ItachiRobot.modules.sql.notes_sql as sql

# from ItachiRobot.modules.rules import get_rules
import ItachiRobot.modules.sql.rules_sql as rulessql
from ItachiRobot import EVENT_LOGS,OWNER_ID, SUPPORT_CHAT, dispatcher
from ItachiRobot.__main__ import DATA_IMPORT
...
from ItachiRobot.modules.sql import disable_sql as disabledsql
...
def export_data(update, context):
    ...
    with open("ItachiRobot{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("ItachiRobot{}.backup".format(chat_id), "rb"),
        caption="📤*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `ItachiRobot-Backup` was specially made for notes 📚.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("ItachiRobot{}.backup".format(chat_id))  # Cleaning file
