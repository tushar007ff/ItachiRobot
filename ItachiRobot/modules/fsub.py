...
from ItachiRobot import BOT_ID
from ItachiRobot import DRAGONS as DEVS
from ItachiRobot import OWNER_ID
from ItachiRobot import telethn as Rudra
from ItachiRobot.events import Rudrainline
from ItachiRobot.events import register as Rudrabot
from ItachiRobot.modules.no_sql import fsub_db as db


async def is_admin(chat_id, user_id):
    try:
        p = await Rudra(GetParticipantRequest(chat_id, user_id))
...
async def participant_check(channel, user_id):
    try:
        await Rudra(GetParticipantRequest(channel, int(user_id)))
        return True
    except UserNotParticipantError:
        return False
    except:
...
@Rudrabot(pattern="^/(fsub|Fsub|forcesubscribe|Forcesub|forcesub|Forcesubscribe) ?(.*)")
...
*ғᴏʀᴄᴇ ꜱᴜʙꜱᴄʀɪʙᴇ:*
   ...
   •➥ *ʀᴜᴅʀᴀʀᴏʙᴏᴛ ᴄᴀɴ ᴍᴜᴛᴇ ᴍᴇᴍʙᴇʀꜱ ᴡʜᴏ ᴀʀᴇ ɴᴏᴛ ꜱᴜʙꜱᴄʀɪʙᴇᴅ ʏᴏᴜʀ ᴄʜᴀɴɴᴇʟ ᴜɴᴛɪʟ ᴛʜᴇʏ ꜱᴜʙꜱᴄʀɪʙᴇ*
